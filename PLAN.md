...
## Goal
Ship the gem without the 82MB of pre-built data and generated struct files. Instead, download the SDE YAML, convert to msgpack, and generate struct definitions automatically when the user runs `gem install sde`.
...
**Flow:**
1. `gem install sde` installs dependencies first (dry-struct, msgpack, etc.)
...
5. Data lands in `GEM_DIR/sde/`, structs in `GEM_DIR/lib/sde/structs/`
6. No changes needed to `lib/sde.rb` — paths are already correct
...
- Requires `SDE::Downloader`, `Dumper`, `StructGenerator`
...
- Exclude `sde/` and `lib/sde/structs/` from `spec.files` (add to reject list)
...
- `git rm -r --cached lib/sde/structs/` to untrack struct files (already in .gitignore but still tracked)

## No Changes Needed
- `lib/sde.rb` — `data_path` already resolves to `../../sde` relative to `__dir__`, which works for both development and installed gem
- `lib/sde/base.rb` — unchanged
- `lib/tasks/sde.rake` — kept for development workflow
- `lib/sde/downloader.rb`, `dumper.rb`, `struct_generator.rb` — unchanged

## Verification
1. `ruby -e "require 'sde'; puts SDE::Type.count"` after `gem install` should load data
2. `rake sde:update` still works for development
3. `gem build sde.gemspec` produces a small gem (no data/struct files)
